...
def create_env(log_direc,task,env_id, client_id, remotes, **kwargs):
    spec = gym.spec(env_id)
    global log_dir,tasks
    log_dir = log_direc
    tasks = task
...
class DiagnosticsInfoI(vectorized.Filter):
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done and self.once:
            ...
            #self.log_dict['episode_reward'] = self._episode_reward
            #self.log.writekvs(self.log_dict)
            self._episode_reward = 0
            self._episode_length = 0
            self._all_rewards = []
        if done:
